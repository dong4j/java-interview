...
Java 经过编译之后生成的 .class 的字节码文件, 运行平台上只要有JVM就能运行, 不需要进行再次编译
...
## 11. [为什么说 Java 中只有值传递](../java/se/value-transfer.md)
...
## 8. 是否可以从一个 static 方法内部发出对非 static 方法的调用
...
## [子类, 父类初始化顺序](../java/se/init_sequence.md)
...
## [什么是单精度和双精度](../java/se/float_double.md)
